...
// TODO: might be a good idea to template Eigen::Vector3f and Eigen::VectorXf
// such that later on we can decide to use doulbe if we want. Double vs float
// might give nontrivial differences..
...
struct RotResidual
{
    ...
    {
        residual[0] = sqrt(
            pow(tgt_(0) - src_(0) * cos(*angle_est) - src_(2) * sin(*angle_est),
                2) +
            pow(tgt_(1) - src_(1), 2) +
            pow(tgt_(2) - src_(2) * cos(*angle_est) + src_(0) * sin(*angle_est),
                2));
        return true;
    }
...
/**
 * Abstract virtual class for decoupling specific scale estimation methods with
 * interfaces.
 */
...
public:
    ...
    /**
     * Pure virtual method for solving scale. Different implementations may have
     * different assumptions about input data.
     * @param src
     * @param dst
    ...
    /**
     * Perform inlier selection based on scale and point distribution. (with cov
     * mats)
     ...
     * @param inliers [out] a row vector of booleans indicating whether a
     * measurement is an inlier
     */
    virtual void distributionInlierSelection(
        const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
        const Eigen::Matrix<double, 3, Eigen::Dynamic>& tgt,
        const std::vector<Eigen::Matrix3d,
                          Eigen::aligned_allocator<Eigen::Matrix3d>>& src_covs,
        const std::vector<Eigen::Matrix3d,
                          Eigen::aligned_allocator<Eigen::Matrix3d>>& tgt_covs,
        double* scale, Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) = 0;
};

/**
 * Abstract virtual class for decoupling specific rotation estimation method
 * implementations with interfaces.
 */
...
public:
    ...
    /**
     * Pure virtual method for solving rotation. Different implementations may
     * have different assumptions about input data.
     * @param src
     * @param dst
    ...
    virtual void solveForRotation(
        ...
        Eigen::Matrix3d* rotation,
        Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) = 0;
    virtual void solveForRotation2D(
...
/**
 * Abstract virtual class for decoupling specific translation estimation method
 * implementations with interfaces.
 */
...
public:
    ...
    /**
     * Pure virtual method for solving translation. Different implementations
     * may have different assumptions about input data.
     * @param src
     * @param dst
...
public:
    ...
    /**
     * Use truncated least squares method to estimate true x given measurements
     * X
    ...
    /**
     * A slightly different implementation of TLS estimate. Use loop tiling to
     * achieve potentially faster performance.
     * @param X Available measurements
     * @param ranges Maximum admissible errors for measurements X
...
public:
     ...
     * measurement is an inlier
     */
    void distributionInlierSelection(
        const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
        const Eigen::Matrix<double, 3, Eigen::Dynamic>& tgt,
        const std::vector<Eigen::Matrix3d,
                          Eigen::aligned_allocator<Eigen::Matrix3d>>& src_covs,
...
/**
 * Perform outlier pruning / inlier selection based on scale. This class does
 * not perform scale estimation. Rather, it estimates outliers based on the
 * assumption that there is no scale difference between the two provided vector
 * of points.
 */
...
public:
    ...
    explicit ScaleInliersSelector(double noise_bound, double cbar2,
                                  double distribution_bound)
        : noise_bound_(noise_bound),
          cbar2_(cbar2),
          distribution_noise_bound_(distribution_bound){};
    /**
...
public:
     ...
     * @param using_median_selection consensus의 median을 고를지 말지 결정하는
     * 파라미터 false이면 weighted mean 값이 리턴됨
     */
...
public:
    ...
    /**
     * Return the cost of the GNC solver at termination. Details of the cost
     * function is dependent on the specific solver implementation.
     *
     * @return cost at termination of the GNC solver. Undefined if run before
     * running the solver.
     */
 ...
 * Rejection,” arXiv:1909.08605 [cs, math], Sep. 2019.
 */
 ...
 * Q.-Y. Zhou, J. Park, and V. Koltun, “Fast Global Registration,” in Computer
 * Vision – ECCV 2016, Cham, 2016, vol. 9906, pp. 766–782. Notice that our
 * implementation differ from the paper on the estimation of T matrix. We only
 * estimate rotation, instead of rotation and translation.
 *
 */
...
public:
     ...
     * algorithms.
     *
     * GNC_TLS: see H. Yang, P. Antonante, V. Tzoumas, and L. Carlone,
     * “Graduated Non-Convexity for Robust Spatial Perception: From Non-Minimal
     * Solvers to Global Outlier Rejection,” arXiv:1909.08605 [cs, math], Sep.
     * 2019.
     *
     * FGR: see Q.-Y. Zhou, J. Park, and V. Koltun, “Fast Global Registration,”
     ...
     * filtering.
     *
     * CHAIN: formulate TIMs by only calculating the TIMs for consecutive
     * measurements COMPLETE: formulate a fully connected TIM graph
     */
    ...
    /**
     * A struct representing params for initializing the
     * RobustRegistrationSolver
     *
     * Note: the default values needed to be changed accordingly for best
     * performance.
     */
    struct Params
    {
         ...
         * scale differences between the src and dst points. If set to True, the
         * solver will first solve for scale.
         *
         * When the solver does not estimate scale, it solves the registration
         * problem much faster.
         */
         ...
         * every iteration. For GNC-TLS: the algorithm multiples the control
         * parameter by the factor every iteration.
         */
         ...
         *
         * For FGR / GNC-TLS algorithm, the cost thresholds represent different
         * meanings. For FGR: the cost threshold compares with the computed cost
         * at each iteration For GNC-TLS: the cost threshold compares with the
         * difference between costs of consecutive iterations.
         */
        ...
        INLIER_GRAPH_FORMULATION rotation_tim_graph =
            INLIER_GRAPH_FORMULATION::CHAIN;
        ...
        /**
         * \brief The threshold ratio for determining whether to skip max clique
         * and go straightly to GNC rotation estimation. Set this to 1 to always
         * use exact max clique selection, 0 to always skip exact max clique
         * selection.
         *
         * \attention Note that the use_max_clique option takes precedence. In
         * other words, if use_max_clique is set to false, then
         * kcore_heuristic_threshold will be ignored. If use_max_clique is set
         * to true, then the following will happen: if the max core number of
         * the inlier graph is lower than the kcore_heuristic_threshold as a
         * percentage of the total nodes in the inlier graph, then the code will
         * preceed to call the max clique finder. Otherwise, the graph will be
         * directly fed to the GNC rotation solver.
         *
         */
         ...
         * Set this to true to enable max clique inlier selection, false to skip
         * it.
         */
     ...
     * accordingly.
     *
     * This is the preferred way of initializing the different estimators,
     * instead of setting each estimator one by one.
     * @param params
     */
    RobustRegistrationSolver(const Params& params);

    void getTargetNormals(
        Eigen::Matrix<double, 3, Eigen::Dynamic>& target_normals)
    {
    ...
    /**
     * Given a 3-by-N matrix representing points with covariances, return
     * distribution-based Translation Invariant Measurements (TIMs)
     ...
     */
    Eigen::Matrix<double, 3, Eigen::Dynamic> computeTIMsWithCov(
        const Eigen::Matrix<double, 3, Eigen::Dynamic>& v,
        const std::vector<Eigen::Matrix3d,
                          Eigen::aligned_allocator<Eigen::Matrix3d>>& cov,
        std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>>&
            covtiled,
        Eigen::Matrix<int, 2, Eigen::Dynamic>* map);

    /**
     * Given a 3-by-N matrix representing points, return Translation Invariant
     * Measurements (TIMs)
     * @param v a 3-by-N matrix
     ...
     * @param correspondences A vector of tuples representing the
     * correspondences between pairs of points in the two clouds
     */
    ...
    /**
     * Solve for scale, translation and rotation with covariance pruning.
     * Assumes v2 is v1 after transformation.
     * @param v1
     * @param v2
    ...
    /**
     * Solve for scale, translation and rotation. Assumes v2 is v1 after
     * transformation.
     * @param v1
     * @param v2
     */
     ...
     * @param estimator
     */
    inline void setScaleEstimator(
        std::unique_ptr<AbstractScaleSolver> estimator)
    {
     ...
     *
     * Note: due to the fact that rotation estimator takes in a noise bound that
     * is dependent on the estimated scale, make sure to set the correct params
     * before calling solve.
     * @param estimator
     */
    inline void setRotationEstimator(
        std::unique_ptr<GNCRotationSolver> estimator)
    {
     ...
     * @param estimator
     */
    inline void setTranslationEstimator(
        std::unique_ptr<AbstractTranslationSolver> estimator)
    {
     ...
     * measurements are inliers according to scales.
     *
     ...
     * TIMs).
     *
     * @return a 2-by-(number of TIMs) Eigen matrix. Entries in one column
     * represent the indices of the two measurements used to calculate the
     * corresponding TIM.
     */
     ...
     *
     * @return a vector of tuples. Entries in each tuple represent the indices
     * of the two measurements used to calculate the corresponding TIM:
     ...
     *
     * @return a 1-by-(size of max clique) Eigen matrix. Entries represent the
     * indices of the original measurements.
     */
    inline Eigen::Matrix<int, 1, Eigen::Dynamic> getRotationInliersMap()
    {
        Eigen::Matrix<int, 1, Eigen::Dynamic> map =
            Eigen::Map<Eigen::Matrix<int, 1, Eigen::Dynamic>>(
     ...
     *
     * @return a vector of indices of TIMs passed to rotation estimator deemed
     * as inliers by rotation estimation. Note that depending on the
     * rotation_tim_graph parameter, number of TIMs passed to rotation
     ...
     *
     * @return a 1-by-(size of max clique) boolean Eigen matrix. It is
     * equivalent to a binary mask on the inlier max clique, with true
     * representing that the measurement is an inlier after translation
     * estimation.
     */
     ...
     * Return inliers from rotation estimation
     *
     * @return a vector of indices of measurements deemed as inliers by rotation
     * estimation
     */
    ...
    void reset(const Params& params)
    {
        ...
        { // quatro_examples의 setparams 에서 false로 해줌 =>
          // ScaleInliersSelector를 가리키는 스마트 포인터  scale_solver_ ->
          // (ScaleInliersSelector)
            setScaleEstimator(std::make_unique<teaser::TLSScaleSolver>(
                params_.noise_bound, params_.cbar2));
        }
        else
        {
            setScaleEstimator(std::make_unique<teaser::ScaleInliersSelector>(
                params_.noise_bound, params_.cbar2,
                params_.distribution_noise_bound));
        }

        // Initialize the rotation estimator
        //    std::cout<<"\033[1;33m What? "<<
        //    params_.noise_bound<<"\033[0m"<<std::endl;
        ...
        switch (params_.rotation_estimation_algorithm)
        {
            case ROTATION_ESTIMATION_ALGORITHM::GNC_TLS: { // GNC-TLS method
                ...
                setRotationEstimator(
                    std::make_unique<teaser::GNCTLSRotationSolver>(
                        rotation_params));
                break;
            }
            case ROTATION_ESTIMATION_ALGORITHM::FGR: { // FGR method
                setRotationEstimator(
                    std::make_unique<teaser::FastGlobalRegistrationSolver>(
                        rotation_params));
                break;
            }
        }
        ...
        setTranslationEstimator(std::make_unique<teaser::TLSTranslationSolver>(
            params_.noise_bound, params_.cbar2));
