...
# CUDA_VISIBLE_DEVICES=1 python3 /home/server01/js_ws/lidar_test/train_from_supervised_impulse.py --resume-from /home/server01/js_ws/lidar_test/ckpt/supervised_learned.pth

PY_CMD="env CUDA_VISIBLE_DEVICES=1 python3 /home/server01/js_ws/lidar_test/train_encoding_impulse.py \
...
function run_and_monitor {
    while true; do
        ...
        IDLE_THRESHOLD=360   # how many consecutive seconds of lower than 10% usage before restart
        ...
        while kill -0 $TRAIN_PID 2>/dev/null; do
            ...
            if [ $GPU_USAGE_INT -le 5 ]; then
                IDLE_COUNT=$((IDLE_COUNT+1))
                # echo "[Monitor Script] gpu working less than 5... current count: $IDLE_COUNT"
            else
                IDLE_COUNT=0
            fi
